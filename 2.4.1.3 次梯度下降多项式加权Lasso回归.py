...
if __name__ == "__main__":
    ...
    model = LinearRegression(GD_max_steps = 3000)
    model.train(X_train_processed, y_train, regressionType=model.RegressionType.LassoRegression,
                soulutionType=model.SoulutionType.SGD, processingType=model.ProcessingType.not_process,weights=weights)
    y_predict = model.predict(
        X_test_processed, processingType=model.ProcessingType.not_process,weights=weights)

    lasso_model = Lasso(alpha=0.005)
    lasso_model.fit(X_train_processed, y_train,sample_weight=weights)
