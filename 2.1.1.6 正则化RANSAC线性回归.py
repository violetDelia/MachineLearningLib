...
if __name__ == "__main__":
    X_train, y_train = generate_ransac_sample(1000,50)
    X_test, y_test = generate_sample(1000)

    model = LinearRegression(f_test_confidence_interval=0.55)
    model.RANSAC(X_train, y_train, 1,300, 0.5, 100, regressionType=model.RegressionType.LinearRegression,
                 soulutionType=model.SoulutionType.normal, processingType=model.ProcessingType.multinomial,
                 processing_feature_degree=10, featureSelectionType=None)
